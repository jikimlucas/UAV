    ...
    public List<VideoDownload> extractLinks(final VideoInfo info, final AtomicBoolean stop, final Runnable notify) {
        ...
        try {
            ...
            if (data.request.files.h264.hd != null)
                list.add(new VideoDownload(VimeoInfo.VimeoQuality.pHi, new URL(data.request.files.h264.hd.url)));

            if (data.request.files.h264.sd != null)
                list.add(new VideoDownload(VimeoInfo.VimeoQuality.pLow, new URL(data.request.files.h264.sd.url)));
